...
const notFound = (path: string): Either<ParseErrors, never> => ko(new NotFound(path));
...
const getInterfaceProperty = (a: ts.PropertySignature): m.Property => {
  ...
  const annotation = getAnnotation(a.getJsDocs());
  //const description = getDescription(a.getJsDocs());

  const type = getType(a.getType());
  return ({
    kind: 'Property', name, type, signature, ...getProp(annotation)
  });
};
...
const getTitle = (annotation: Annotation): string | undefined =>
  fromNullable(getTag('title')(annotation)).map(a => fromJSDocDescription(a.description)).toUndefined();
...
const parsePropertyinterface = (i: ts.InterfaceDeclaration): ParseResult<m.Export> => {
  return new ReaderEither(e => {
    ...
    if (isPropertyinterface(annotation)) {
      ...
      const result: m.Propertyinterface = { kind: 'Propertyinterface', name, signature, ...getProp(annotation), items };
      return ok(result);
    }
    return notFound('');
  });
};
const getProp = (annotation: Annotation) => {
  const description = fromJSDocDescription(annotation.description);
  const header = getHeader(annotation);
  const title = getTitle(annotation);
...
const parseModuleInterface = (i: ts.InterfaceDeclaration): ParseResult<m.Export> => {
  return new ReaderEither(() => {
    ...
    if (isModule(annotation)) {
      ...
      /*       const description = fromJSDocDescription(annotation.description);
            const header = getHeader(annotation); */
      const result: m.DocModule = { kind: 'Module', name, signature, ...getProp(annotation) };
      return ok(result);
    }
    return notFound('');
  });
